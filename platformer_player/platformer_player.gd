...
var on_door = false
@export var max_floating_mana : float = 10

@onready var double_jump = moveDate.double_jump_count

@onready var floating_mana = max_floating_mana
...
func _process(delta: float) -> void:
	texture_progress_bar.value = floating_mana
	texture_progress_bar.visible = floating_mana < max_floating_mana
...
func floating():
	if Input.is_action_pressed("jump") && velocity.y > 0 && double_jump >= 0 && floating_mana > 0:
		velocity.y = 0
		floating_mana -= 0.1

func reset_floating_mana():
	floating_mana = max_floating_mana
